...
(define sqrt-with
  (lambda (x y)
        (cons-lzl y
              (lambda () (sqrt-with x (// (++ (** y y) x) (** (as-real 2) y)))))
  )
)
...
;;; Help function for Q2.2.b
(define diagHelp
  (lambda (lzl n)
    (cons-lzl (nth (head lzl) n)
              (lambda() (diagHelp (tail lzl) (+ n 1))))
  )
)
