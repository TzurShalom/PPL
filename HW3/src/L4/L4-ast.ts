...
const isSpecialFormKeyword = (x: string): x is SpecialFormKeyword => ["if", "lambda", "let", "quote", "letrec", "set!", "bound?", "time"].includes(x);
...
// L4
export const makeLetrecExp = (bindings: Binding[], body: CExp[]): LetrecExp => ({tag: "LetrecExp", bindings: bindings, body: body});
export const makeSetExp = (v: VarRef, val: CExp): SetExp => ({tag: "SetExp", var: v, val: val});

// HW3
export const makeBoundExp = (v: VarRef): BoundExp => ({tag: "BoundExp", var: v});
export const makeTimeExp = (e: CExp): TimeExp => ({tag: "TimeExp", exp: e});
...
const parseBoundExp = (params: Sexp[]): Result<BoundExp> =>
    params.length < 1 ? makeFailure('bound? has too few arguments') :
    params.length > 1 ? makeFailure('bound? has too many arguments') :
    ! isIdentifier(params[0]) ? makeFailure(`The arg of bound? must be an identifier: ${format(params[0])}`) :
    makeOk(makeBoundExp(makeVarRef(params[0])));

const parseTimeExp = (params: Sexp[]): Result<TimeExp> => 
    params.length < 1 ? makeFailure('time has too few arguments') :
    params.length > 1 ? makeFailure('time has too many arguments') :
    mapv(parseL4CExp(params[0]), (param: CExp) => makeTimeExp(param))
