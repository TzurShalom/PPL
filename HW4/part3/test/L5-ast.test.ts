...
import { TExp, isProcTExp, isUnionTExp, makeBoolTExp, makeNumTExp, makeStrTExp, makeUnionTExp, parseTE, parseTExp } from "../src/L5/TExp";
...
describe('L5 parseTExp Union Parser', () => {
    ...
    it('parseTExp parses embedded union expressions', () => {
        // todo
        expect(parseTE("(union boolean (union number string))")).toEqual(makeOk(makeUnionTExp([makeBoolTExp(), makeUnionTExp([makeNumTExp(), makeStrTExp()])]))); 
    });
    ...
    it('parseTExp fails to parse union of bad type expressions', () => {
        // todo
        expect(parseTE("(union number)")).toEqual(makeFailure("fail to parse union")); 
    });
...
describe('L5 Unparse', () => {
    ...
    it('unparses union, nested unions in different positions', () => {
        // TODO
        const union = "(union string (union boolean number))";
