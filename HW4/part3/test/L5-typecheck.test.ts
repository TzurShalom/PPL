...
import { isProgram, makeStrExp, parseL5, Program } from '../src/L5/L5-ast';
...
describe('L5 Type Checker', () => {
    ...
    describe('L5 Test checkCompatibleType with unions', () => {
        // TODO L51
        expect(checkCompatibleType(makeNumTExp(), makeNumTExp(), makeStrExp("testA"))).toEqual(makeOk(true));
        expect(checkCompatibleType(makeNumTExp(), makeUnionTExp([makeNumTExp(), makeBoolTExp()]), makeStrExp("testB"))).toEqual(makeOk(true));
        expect(checkCompatibleType(makeUnion(makeNumTExp(), makeBoolTExp()), makeUnion(makeNumTExp(), makeBoolTExp()), makeStrExp("testC"))).toEqual(makeOk(true));
    });
    ...
    describe('L5 Test makeUnion', () => {
        ...
        expect(makeUnion(makeNumTExp(), makeUnion(makeNumTExp(), makeBoolTExp()))).toEqual(makeUnionTExp([makeBoolTExp(), makeNumTExp()]))
    ...
    describe('L5 Test typeOfIf with union in all relevant positions', () => {
        ...
        expect(L5typeof('(if #t 1 #t)')).toEqual(makeOk("(union BoolTExp NumTExp)"));
        // typeOfIf( (if #t 1 2) ) -> number
        expect(L5typeof('(if #t 1 2)')).toEqual(makeOk("number"));
        // typeOfIf( (if #t "ok" (if #f 1 #t)) ) -> union(boolean, number, string)
        expect(L5typeof('(if #t "ok" (if #f 1 #t))')).toEqual(makeOk("(union BoolTExp (union NumTExp StrTExp))"));
        // typeOfIf( (if #t (if #f 1 #t) "ok") ) -> union(boolean, number, string)
        expect(L5typeof('(if #t (if #f 1 #t) "ok")')).toEqual(makeOk("(union BoolTExp (union NumTExp StrTExp))"));
    ...
    describe('L5 Test checkCompatibleType with unions in arg positions of Procedures', () => {
        ...
        // example 1 at work file
        expect(L5typeof('(lambda ((x : (union number boolean))) : string"F OK")')).toEqual(makeOk("((union BoolTExp NumTExp) -> string)"));
        // example 2 at work file
        expect(L5typeof('(lambda ((x : number)) : string "G OK")')).toEqual(makeOk("(number -> string)"));
        // example 3 at work file
        expect(L5typeof('(lambda ((t : (number -> string))) : string (t 1))')).toEqual(makeOk("((number -> string) -> string)"));
        // example 4 at work file
        expect(L5typeof('(lambda ((t : ((union number boolean) -> string))) : string (t #t))')).toEqual(makeOk("(((union BoolTExp NumTExp) -> string) -> string)"));
    });
});
