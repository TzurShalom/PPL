...
const parseCompoundTExp = (texps: Sexp[]): Result<CompoundTExp> => {
    ...
    return (pos === -1)  ? 
                texps[0] === "union" ? bind(parseUnionTExp(texps.slice(1)), (args: TExp[]) =>
...
const parseUnionTExpHelp = (texps: Sexp[]): TExp[] => {
    let ret: TExp[] = [];
    if(texps.length != 2) makeFailure("fail to parse union");
    const r1 = parseTExp(texps[0]);
    if(isOk(r1)){
        isArray(r1.value) ? ret = ret.concat(r1.value) : ret.push(r1.value);
    }
    const r2 = parseTExp(texps[1]);
    if(isOk(r2)){
        isArray(r2.value) ? ret = ret.concat(r2.value) : ret.push(r2.value);
    }
    const uniqueArray = ret.filter((value, index, self) => self.indexOf(value) === index);
    return uniqueArray;
}

// parse parseTExp on fisrt parse on second- if one of them in union take the conp.s to array
const parseUnionTExp = (texps: Sexp[]): Result<TExp[]> =>
    (texps.length < 2) ? makeFailure("fail to parse union") :
    makeOk(parseUnionTExpHelp(texps).sort((a, b) => JSON.stringify(a).localeCompare(JSON.stringify(b))));
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const unparseUnionString = (paramTes:TExp[]): string =>{
        let str = "";
        paramTes.length === 2 ?
            str += "(union " + paramTes[0].tag + " " + paramTes[1].tag + ")" :
            str += "(union " + paramTes[0].tag + " " + unparseUnionString(paramTes.slice(1)) + ")";
        return str;
    }

    const unparseUnion = (paramTes:TExp[]): Result<string> => {
        return isNonEmptyList<TExp>(paramTes) ?
            makeOk(unparseUnionString(paramTes)) :
            makeOk("Empty list in union");
    }
