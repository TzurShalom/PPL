...
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isUnionTExp, UnionTExp, makeUnionTExp, isBoolTExp, isNumTExp, isStrTExp } from "./TExp";
...
export const checkCompatibleType = (te1: TExp, te2: TExp, exp: Exp): Result<true> =>
  ...
  isUnionTExp(te2) ? 
    (isUnionTExp(te1) ? checkComp2(te1, te2) : checkComp1(te1, te2)) :
...
export const checkComp1 = (te1: TExp, te2: UnionTExp): Result<true> => {
    for (let index = 0; index < te2.components.length; index++) {
        if(te1.tag === te2.components[index].tag) {return makeOk(true);}
    }
    return makeFailure<true>("te1 element is not in te2 #1");
}

export const checkComp2 = (te1: UnionTExp, te2: UnionTExp): Result<true> =>{
    for (let index = 0; index < te1.components.length; index++) {
        if(!te2.components.includes(te1.components[index])) {return makeFailure<true>("te1 element is not in te2 #2");}
    }
    return makeOk(true);
}
...
export const typeofExp = (exp: Parsed, tenv: TEnv): Result<TExp> =>
    ...
    isUnionTExp(exp) ? typeOfUnion(exp) :
    isDefineExp(exp) ? typeofDefine(exp, tenv) :
    isProgram(exp) ? typeofProgram(exp, tenv) :
    makeFailure(`Unknown type: ${format(exp)}`);

// L5 Union
export const typeOfUnion = (exps: TExp[]): Result<TExp> => {
    return makeOk(makeUnionTExp(exps));
}
...
export const makeUnion = (te1: TExp, te2: TExp): TExp =>{
    ...
    // if they equals
    if(isBoolTExp(te1) && isBoolTExp(te2)) {return makeBoolTExp();}
    if(isNumTExp(te1) && isNumTExp(te2)) {return makeNumTExp();}
    if(isStrTExp(te1) && isStrTExp(te2)) {return makeStrTExp();}
    // if union
    // remove duplicates
    // if proc exp????
    let arr: any[] = [];
    isUnionTExp(te1) ? 
        arr = arr.concat(te1.components) : arr.push(te1);
    isUnionTExp(te2) ? 
        arr = arr.concat(te2.components) : arr.push(te2);
    arr.sort((a,b) => a.tag.localeCompare(b.tag));
    const uniqueArray = arr.filter((value, index, self) => {
        const tag = value.tag;
        return index === self.findIndex((c) => c.tag === tag);
    })
    return makeUnionTExp(uniqueArray);
}
...
export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> => {
    ...
    // isOk(bind(thenTE, (thenTE: TExp) => bind(altTE, (altTE: TExp) =>
    //         checkCompatibleType(thenTE, altTE, ifExp)))) ? thenTE :
    //         (bind(thenTE, (thenTE: TExp) => bind(altTE, (altTE: TExp) =>
    //         retTE = makeOk(makeUnion(thenTE, altTE)))));
    // isOk(bind(thenTE, (thenTE: TExp) => bind(altTE, (altTE: TExp) =>
    //         checkCompatibleType(thenTE, altTE, ifExp)))) ? altTE :
    //         (bind(altTE, (altTE: TExp) => bind(thenTE, (thenTE: TExp) =>
    //         retTE = makeOk(makeUnion(thenTE, altTE)))));
    // return bind(constraint1, (_c1: true) =>
    //             retTE);
    const constraint2 = bind(thenTE, (thenTE: TExp) =>
        bind(altTE, (altTE: TExp) => checkCompatibleType(thenTE, altTE, ifExp).tag === "Ok" ?
            makeOk(thenTE) : makeOk(makeUnion(thenTE, altTE))));
...
export const typeofDefine = (exp: DefineExp, tenv: TEnv): Result<TExp> => {
    const valueExp = exp.val;
    const varDecl = exp.var;
    const valueTE: Result<TExp> = typeofExp(valueExp, tenv);
    const updateTEnv = (valueTE: TExp): Result<TExp> => {
      const extendedTEnv: TEnv = makeExtendTEnv([varDecl.var], [valueTE], tenv);
      return bind(checkCompatibleType(varDecl.texp, valueTE, valueExp), (_) =>
        makeOk(makeVoidTExp())
      );
    };
    
    return bind(valueTE, updateTEnv);
};
